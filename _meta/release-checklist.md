...
6. Updates: the workflow signed the DMG and attached `appcast.xml` to the release; check
   ...
   `site` job has redeployed (it calls pages.yml with this tag; the release event alone never would, and "latest" can still point at the previous release for a moment). Installed apps see the update within a day (or on Check for Updates…).
