...
| `Updates.swift` | About → Check for updates: `parse` (release JSON → version, page, DMG), `isNewer` (numeric per component; `git describe` suffixes ignored), `systemFetch` (10 s GET, no cache). The check runs only on the user's click; the app never contacts the network otherwise. It links to the DMG — no download, no install (Sparkle would be the next step if drag-to-Applications proves to be skipped). |
