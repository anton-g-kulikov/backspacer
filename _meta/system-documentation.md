...
| Piece | Role |
|---|---|
| `catalog.json` | The knowledge: what to measure, which bucket it belongs to, how to delete it, what to warn about. `catalog.schema.json` (JSON Schema 2020-12) describes it — field docs, the bucket enum, and the cross-field rules (`children` ⇒ single `path`, `itemsCmd` ⇔ `deleteItemCmd` with `{key}`, every entry has something to measure or show). Every entry carries a one-line `note` and an `explain` block (`what`, `why`, `after`, `keep`) — the reasoning Details shows; C17 enforces both. Editors validate on the `$schema` line; `SchemaTests` validate in `swift test`. Since ADR-22 step 1 an entry may carry `platforms` (hosts that show it; absent = macOS only) and `os.linux` / `os.windows` overrides of its location and command fields, with `$HOME`/XDG/`%APPDATA%`-style variables; 40 entries are marked portable. Base fields are macOS; a Linux- or Windows-only entry keeps its location and commands in `os.<platform>` and has no base path, so `Catalog.load` hides it on the Mac (`isForMacOS`, C16) while `loadAll` keeps it for the catalog-wide tests; the macOS safety gate (S7) checks base paths only — per-OS gates come with the hosts. Ten platform-only entries exist (apt/dnf/snap caches, thumbnails, Trash and the journal on Linux; temp, Windows Update, Delivery Optimization and crash dumps on Windows). `Tests/web/catalog.test.js` holds the platform invariants. |
| `web/index.html` | The UI: markup and the two theme stylesheets. Carries a Content-Security-Policy (`script-src 'self'`, no inline script), so nothing that reaches the DOM as text can execute. Runs standalone in a browser with a mock bridge. The header's `.brand` block leads with the app icon (`web/icon.svg`, a copy of `assets/AppIcon.svg` kept fresh by `scripts/make-icon.sh`; hidden in Terminal). Glass: the header and footer are frosted bands the list scrolls under (absolute over `main`, `--band` + backdrop blur; `main` pads by their observed heights with 26 px of clear space each side; the list's left pad equals the permanent scrollbar track so header, islands and footer share edges). |
| `web/boot.js` | Picks the cached theme before first paint. |
| `web/app.js` | Binds page state to the DOM and the bridge: rendering, scanning, the confirm flows, project folders, About. Mouse-down policy: on the header it asks the bridge to drag the window; elsewhere, outside selectable text (paths, Details output, panels) and controls, the default is suppressed so a drag cannot sweep a selection across the paths — a path is copied by dragging inside it. The size threshold (default 10 MB) hides rows and, inside Details, items below it — a last row names how many and how much and reveals them. The header tagline carries the reclaimable total (`taglineText`; Safe + Regenerable + Your call, counted as the bucket badges are) and is rewritten by `updateTotals`; the scan verbs cycle in the Scan button itself (`startScanWords`). A delete in flight says so (R28a): the instant the call goes out the row's size cell reads
`deleting…` and blinks (`setBusy`; the word carries it when `prefers-reduced-motion` drops the
blink, as `startScanWords` does), the row is `aria-busy`, its actions dim, and the footer button
counts `Deleting 2 of 5…` (`deletingLabel`); each item announces when it starts, not only when it
ends. `state.deleting` is the re-entry guard — a second click cannot start a second pass, and it
also holds `updateTotals` off `#deleteSel.disabled` so the busy label survives the loop. Nothing
is disabled: disabling the control that has focus drops a screen reader to the body, which is why
Scan uses `aria-busy` too (A4). All three loops — rows, one item, one project's output — behave
the same, and a failed delete restores the row it found.
The by-project view (`By tool | By project` in the Project folders island): after a scan the page asks `projects` and folds the `$PROJECTS` entries' items into their project by longest path prefix (`groupByProject`), stalest first with the age in words (`ago`, red past six months) or largest first — a Stalest | Largest segment in the card's header (`sortProjects`, the `projectSort` preference; Elsewhere stays last either way); a project's Delete is one confirmation then the existing per-item `delete` for each of its items — nothing the by-tool view could not delete. In that view the `$PROJECTS` entries leave the buckets (rows, group label, select-all and badges all follow `visible()`), so nothing is shown twice; the tagline adds the card's bytes back, so it reads the same in both views. |
...
| `Catalog.swift` | Typed mirror of `catalog.json`; `Resources` locates bundled files. ADR-22: `Entry` decodes `platforms` and `os` overrides; `load(from:)` keeps entries that list macOS (or say nothing) and rebuilds `rawJSON` from that filtered array, so the page never sees a Linux/Windows-only entry; `loadAll(from:)` is for the catalog-wide tests; the Mac host ignores `os.*`. |
...
Details opens with the entry's **Explain** block (`explainHTML` in `logic.js`,
a `<dl>` of labelled lines): *What* lives there, *Why <bucket>* — the reasoning
behind the bucket — *After deleting* — what regrows, what is re-downloaded,
what is lost — and *Keep if*. It comes from the entry's `explain` object in
`catalog.json`; every entry has one and a one-line `note` (C17 refuses a
catalog without), so every row has a Details button. The block sits above the
