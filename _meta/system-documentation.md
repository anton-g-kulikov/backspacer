...
| `web/index.html` | The UI: markup and the two theme stylesheets. Carries a Content-Security-Policy (`script-src 'self'`, no inline script), so nothing that reaches the DOM as text can execute. Runs standalone in a browser with a mock bridge. The header's `.brand` block leads with the app icon (`web/icon.svg`, a copy of `assets/AppIcon.svg` kept fresh by `scripts/make-icon.sh`; hidden in Terminal). |
